...
class QuadtreeContour2D:
    ...
    def contour(
        ...
        ymax=1.0,
        max_depth=10,
        min_depth=2,          # force a little refinement
        min_size=None,
        use_midpoints=True,   # sample edge midpoints + center for intersection test
    ):
        if min_size is None:
...
def compute_persistence_diagrams_2d(field2d: np.ndarray, superlevel: bool = False):
    """
    field2d: 2D numpy array
    superlevel=False -> sublevel persistence (basins)
    superlevel=True  -> superlevel persistence (peaks) by negating the field
    returns: (dgm0, dgm1) as numpy arrays of shape [K,2] with (birth, death)
    """
    if not HAVE_GUDHI:
        raise RuntimeError("gudhi is not installed. `pip install gudhi`")
    ...
    # Drop infinite deaths (common for final H0 component)
    def drop_inf(dgm):
        if dgm.size == 0:
            return dgm
...
def plot_two_pd(dgmA, dgmB, titleA="Truth", titleB="INR", suptitle="Persistence diagrams"):
    ...
    def plot_one(ax, dgm, title):
        ...
        ax.scatter(b, d, s=10)
        ax.plot([mn, mx], [mn, mx], linewidth=1)  # diagonal
        ax.set_xlim(mn, mx)
        ax.set_ylim(mn, mx)
...
def diagram_distances(dgmA, dgmB):
    """
    Returns bottleneck and Wasserstein-1 distances if persim is available.
    """
    if not HAVE_PERSIM:
        return None
...
def main():
    ...
    # ------------------------------------------------------------
    # Persistence diagrams: compare truth vs INR render
    # ------------------------------------------------------------
    # IMPORTANT: compute PDs on the SAME resolution.
    # Here we upsample the truth (64x64) to 256x256 and compare to t_r (256x256).
    if HAVE_GUDHI:
        x0_up_np = x0_up.squeeze().detach().cpu().numpy()  # [256,256]
        inr_np = t_r.astype(np.float64)                    # [256,256]

        # Sublevel persistence (basins). For peaks/topology of maxima, set superlevel=True.
        superlevel = False

        dgm0_truth, dgm1_truth = compute_persistence_diagrams_2d(x0_up_np, superlevel=superlevel)
        dgm0_inr, dgm1_inr = compute_persistence_diagrams_2d(inr_np, superlevel=superlevel)

        print(f"\nPersistence diagrams ({'superlevel' if superlevel else 'sublevel'}):")
        print(f"  PD sizes: H0 truth={len(dgm0_truth)} H0 inr={len(dgm0_inr)}   "
              f"H1 truth={len(dgm1_truth)} H1 inr={len(dgm1_inr)}")

        plot_two_pd(
            dgm0_truth, dgm0_inr,
            titleA="Truth (H0)", titleB="INR (H0)",
            suptitle="Persistence Diagram H0 (components)"
        )

        plot_two_pd(
            dgm1_truth, dgm1_inr,
            titleA="Truth (H1)", titleB="INR (H1)",
            suptitle="Persistence Diagram H1 (loops)"
        )

        if HAVE_PERSIM:
            d0 = diagram_distances(dgm0_truth, dgm0_inr)
            d1 = diagram_distances(dgm1_truth, dgm1_inr)
            print("  Diagram distances:")
            print("    H0:", d0)
            print("    H1:", d1)
        else:
            print("  Install persim for distances: pip install persim")
    else:
        print("\nInstall gudhi for persistence diagrams: pip install gudhi")
